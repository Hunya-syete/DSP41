...
triangle_signal = TriangleSignal(freq=620)
wave = triangle_signal.make_wave(duration=0.01)
wave.plot()
plt.title("Triangle Waveform (620 Hz)")
plt.xlabel("Time (s)")
...
sawtooth_chirp = SawtoothChirp(start=400, end=900)
...
wave_chirp.make_spectrogram(1082).plot()
plt.show()
